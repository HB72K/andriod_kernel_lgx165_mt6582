...
#define NMI_RESET_PIN		GPIO114 //v10
//2014/09/30-joubertshe-44857-[5513][Drv]DTV eint gpio updated
#define NMI_IRQN_PIN		GPIO115 //v10

//joubertshe add eint_gpio of dtv
#define GPIO_DVT_EINT_PIN 	GPIO115
//2014/09/30-joubertshe-44857
#define GPIO_DVT_EINT_PIN_M_GPIO GPIO_MODE_00
#define GPIO_DVT_EINT_PIN_M_EINT GPIO_MODE_02
...
/***************************************/
#define CUST_EINT_DTV_NUM				CUST_EINT_CUST_DTV_NUM		// mediatek\custom\lge75_v4_jb\kernel\dct\dct\custo_eint.h
#define CUST_EINT_DTV_DEBOUTCE_CN		CUST_EINT_CUST_DTV_DEBOUNCE_CN
...
{
		//2014/09/30-joubertshe-44569-[5513][Drv]DTV kernel driver updated of power
        if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200,NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable digital power\n");
            return -EIO;
        }      
		//2014/09/30-joubertshe-44569-
...
void isdbt_gpio_power_off(void)
{
	//mt_set_gpio_out(NMI_POWER_PIN, 0);
    	//2014/09/30-joubertshe-44569-[5513][Drv]DTV kernel driver updated of power
        if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D, NMI_HW_DRVNAME))
        {
            NM_KMSG("[ISDBT] Fail to enable digital power\n");
            return -EIO;
        }
...
void isdbt_gpio_interrupt_register(void)
{
   ...
   // NM_KMSG("<isdbt> isdbt_gpio_interrupt_register, (%d)[%d]\n",GPIO_DVT_EINT_PIN,CUST_EINT_DTV_NUM);
	...
	// irq register
	//mt_eint_registration(CUST_EINT_DTV_NUM, CUST_EINT_DTV_DEBOUNCE_EN, CUST_EINT_DTV_POLARITY, isdbt_irq_handler, 0);
mt_eint_registration(CUST_EINT_DTV_NUM, EINTF_TRIGGER_FALLING, isdbt_irq_handler, 1);
	// disable irq
	mt_eint_mask(CUST_EINT_DTV_NUM);
...
//<2015/1/9 - joubert.she,[V10][Feature][Common][BSP][][] modify the eint_mask function on mt6582
...
static int __init isdbt_init(void)
{
	...
	if(result)
		return result;
	//<2015/01/26-joubert.she-[V10][Feature][Common][BSP][][] dtv power consumption issue fix 
	isdbt_gpio_power_off();
