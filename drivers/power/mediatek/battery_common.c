...
#if defined(V10_HW)
 kal_uint32  g_batt_lgotp_status = TEMP_POS_NORMAL;
#endif
//>2015/1/26-davidda
...
void mt_battery_GetBatteryData(void)
{
	...
	//<2015/1/26-davidda, [V10][Feature][Common][BSP][]we do not have Rsense
	 #if !defined(V10_HW)
...
{
//<2015/3/18-JichangDu, [V10][Bug][Common][BatteryWarning][][]Add shutdown procedure and fix reboot issue when battery temperature is high.
//<2015/1/26-davidda, [V10][Feature][Common][BSP][]Battery OTP test
#if  !defined(V10_HW)
...
if (KAL_TRUE != BMT_status.charger_exist)
{
	if (BMT_status.temperature >= MAX_BAT_TEMPERATURE ) {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of shut down range\n",
				    BMT_status.temperature);
	}else if(BMT_status.temperature >= ALERT_BAT_TEMERATURE ) {
		g_BatteryNotifyCode |= 0x0002;
...
static void mt_battery_charger_detect_check(void)
{
	if (upmu_is_chr_det() == KAL_TRUE) {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BAT_thread]Cable out \r\n");
                //<2015/1/26-davidda, [V10][Feature][Common][BSP][] Charging IC will not charging without reset,
                #if defined(V10_HW)
                {
      		    U32 charging_enable = false;
                    battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);
...
//<2015/1/30-davidda, [V10][Feature][Common][BSP][]Power on the phone without battery
#if 0//defined(V10_HW)
   if( mt_get_gpio_in(GPIO_BAT_ID))
   		BMT_status.bat_exist = KAL_FALSE ;	/* phone must have battery */
   else
   		BMT_status.bat_exist = KAL_TRUE;	/* phone must have battery */
   	battery_xlog_printk(BAT_LOG_CRTI, "[BAT_probe] BMT_status.bat_exist = %d\n ",
