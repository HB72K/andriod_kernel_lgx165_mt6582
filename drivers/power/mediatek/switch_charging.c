...
static void set_LG_OTP_charging_current(void)
{
	...
	else
	{
		...
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] LG OTP set charging current : %d\r\n",
				    g_temp_CC_value);
    }
}
//>2015/1/26-davidda
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	//<2015/1/20-davidda, [V10][Feature][Common][BSP][]stop charging while battery full
	kal_uint32 charging_enable = KAL_FALSE;
	//>2015/1/20-davidda
	...
	if (   //<2015/2/9-davidda, [V10][Feature][Common][BSP][]enable recharging
                /*charging_full_check() == KAL_FALSE */
		BMT_status.bat_vol < RECHARGING_VOLTAGE|| charging_full_check() == KAL_FALSE 
		//>2015/2/9-davidda
	...
	//<2015/1/20-davidda, [V10][Feature][Common][BSP][]stop charging while battery full
        ...
        {
	       /*  Disable charging */
	       battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY]  Stop charging !!\n\r");
               battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
         }
	//>2015/1/20-davidda
