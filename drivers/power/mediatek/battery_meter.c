...
#endif
//<2015/2/9-davidda, [V10][Feature][Common][BSP][]add temparature diable in hidden manu.
...
void oam_run(void)
{
	...
	//david added for test
		{
		kal_int32 g_rtc_fg_soc_t = 0;

	          g_rtc_fg_soc_t = get_rtc_spare_fg_value();
	bm_print(BM_LOG_CRTI, "[oam_run_time] rtc value =%d\n", g_rtc_fg_soc_t);
		}
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	//<2015/2/11-Leolee, [V10][Feature][Common][OTP][][] Create dirver file temp_on and temp_off for hidden menu interface.
