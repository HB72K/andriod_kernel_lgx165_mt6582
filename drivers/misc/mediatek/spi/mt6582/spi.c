...
static int mt_spi_setup ( struct spi_device *spidev )
{
	...
	if ( !chip_config ) {
		...
		//<2015/01/19-joubert.she [V10][Bug][Common][BSP][][] dtv spi bus timing cmodification
		chip_config->high_time = 8;//10;
		chip_config->low_time = 8;//10;
		//<2015/01/19-joubert.she
		chip_config->cs_idletime = 2;
		chip_config->ulthgh_thrsh = 0;

		chip_config->cpol = 0;
		chip_config->cpha = 0; //1; //joubertshe
