...
typedef struct _epl_raw_data
{
    ...
    u16 als_lux;
    u16 hs_data[200];
	//<2015/01/26-Lever.ni, [V10][Feature][Common][BSP][][]add psensor calibration function
    u16 ps_cal_hdata;
    u16 ps_cal_ctdata;
	//>2015/01/26-Lever.ni
...
#if ELAN_WRITE_CALI
//<2015/01/26-Lever.ni, [V10][Feature][Common][BSP][][]add psensor calibration function
#define PS_CAL_FILE_PATH	"/protect_f/xtalk_cal.dat"  //PS Calbration file path
//>2015/01/26-Lever.ni
...
//<2015/01/26-Lever.ni, [V10][Feature][Common][BSP][][]add psensor calibration function
	fp_cal = filp_open(PS_CAL_FILE_PATH, O_CREAT|O_RDWR|O_TRUNC, 0775);
//>2015/01/26-Lever.ni
	if (IS_ERR(fp_cal))
	{
...
/*CTS --> S_IWUSR | S_IRUGO*/
//<2015/01/26-Lever.ni, [V10][Feature][Common][BSP][][]add psensor calibration function
static ssize_t epl_show_ps_cali_savedata(struct device *dev, struct device_attribute *attr, char *buf) {
    int ps_hthr=0, ps_lthr=0;
    int ps_cal_len = 0;
    char ps_calibration[20];
    struct epl2182_priv * epl_data = epl2182_obj;
    ps_hthr = gRawData.ps_cal_ctdata + (gRawData.ps_cal_hdata - gRawData.ps_cal_ctdata) * 9 /10;
    ps_lthr = gRawData.ps_cal_ctdata + (gRawData.ps_cal_hdata - gRawData.ps_cal_ctdata) * 6 /10;
    
    ps_cal_len = sprintf(ps_calibration, "%d,%d", ps_hthr, ps_lthr);
    
    if(write_factory_calibration(epl_data, ps_calibration, ps_cal_len) < 0)
    {
        APS_ERR("[%s] create file error \n", __func__);
        return -EINVAL;
    }
...
static ssize_t epl_show_ps_sensor_data(struct device *dev, struct device_attribute *attr, char *buf) {
    APS_FUN();
    struct epl2182_priv *obj = epl2182_obj;
    ssize_t len = 0;
    ...
    APS_LOG("epl2182_show_ps_sensor_data =  %d\n", gRawData.ps_raw);
    len += snprintf(buf+len, PAGE_SIZE-len, "%d \r\n", gRawData.ps_raw);
	return len;
}
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(elan_status, 			0644, epl2182_show_status,  NULL);
static DRIVER_ATTR(elan_reg,     		    0644, epl2182_show_reg,   	 NULL);
static DRIVER_ATTR(elan_renvo,    			0644, epl2182_show_renvo,   				NULL);
static DRIVER_ATTR(als_int_time,     	    0644, NULL,   				 epl2182_store_als_int_time);
static DRIVER_ATTR(ps_cal_raw, 			    0644, epl2182_show_ps_cal_raw, 	  		NULL);
static DRIVER_ATTR(ps_int_time,    	        0644, NULL,   				 epl2182_store_ps_int_time);
static DRIVER_ATTR(ps_threshold,     		0644, epl2182_show_ps_threshold, epl2182_store_ps_threshold);
static DRIVER_ATTR(hs_enable,				0644, NULL, epl2182_store_hs_enable);
static DRIVER_ATTR(hs_raws,					0644, epl2182_show_hs_raws, NULL);
static DRIVER_ATTR(hs_polling,				0644, NULL, epl2182_store_hs_polling);
#if ELAN_WRITE_CALI
static DRIVER_ATTR(ps_calfile,				0644, NULL,			epl2182_store_ps_w_calfile);
static DRIVER_ATTR(ps_run_cali,				0644, epl2182_show_ps_run_cali, NULL);
#endif
static DRIVER_ATTR(ps_polling,				0644,  NULL,			epl2182_store_ps_polling);
static DRIVER_ATTR(ps_enable,				0644, NULL,			epl2182_store_ps_enable);
static DRIVER_ATTR(als_enable,				0644, NULL,			epl2182_store_als_enable);
static DRIVER_ATTR(als_data,				0644, epl_show_als_data, NULL);
static DRIVER_ATTR(ps_cali_savedata, 0644, epl_show_ps_cali_savedata, NULL);
static DRIVER_ATTR(ps_sensordata, 0644, epl_show_ps_sensor_data, NULL);
//>2015/01/26-Lever.ni
/*----------------------------------------------------------------------------*/
...
//<2015/01/26-Lever.ni, [V10][Feature][Common][BSP][][]add psensor calibration function
    &driver_attr_ps_cali_savedata,
#endif
    &driver_attr_ps_sensordata,
