...
*
********************************************************************************/
static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, 0x6a>>1)};
...
/*   if (ui4_offset + ui4_length >= 0x2000)
   ...
   */
   ...
   do
   {
       ...
       #if defined(V10_HW)
      // CAM_CALDB("[CAM_CAL][iReadData] Read 0x%x=0x%x,pBuff addr %x\n", u4CurrentOffset, *pBuff,pBuff);
       #else

       CAM_CALDB("[CAM_CAL][iReadData] Read 0x%x=0x%x,pBuff addr %x\n", (int)u4CurrentOffset, (int)*pBuff,(int)pBuff);
       #endif
...
#endif
{
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        CAM_CALDB("[S24CAM_CAL] to user  Working buffer value 0x%x \n", *(u32*)pWorkingBuff);
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    g_pstI2Cclient->addr = S5K5E2OTP_DEVICE_ID>>1;
