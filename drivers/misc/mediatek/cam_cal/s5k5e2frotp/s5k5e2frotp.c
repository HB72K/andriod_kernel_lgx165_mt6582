...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "E2FR_CAM_CAL_DRV"
...
static u32 otp_Start_Addr = 0x0A04;
...
static int iWriteCAM_CAL(u16 a_u2Addr  , u32 a_u4Bytes, u8 puDataInBytes)
{
	...
	do {
        CAM_CALDB("[CAM_CAL_5e2][iWriteCAM_CAL] Write 0x%x=0x%x \n",a_u2Addr, puDataInBytes);
		i4RetValue = i2c_master_send(g_pstI2Cclient, puSendCmd, 3);
        if (i4RetValue != 3) {
            CAM_CALDB("[CAM_CAL_5e2] I2C send failed!!\n");
        }
        else {
            break;
    	}
   ...
   //CAM_CALDB("[CAM_CAL_5e2] iWriteCAM_CAL done!! \n");
    return 0;
}
...
static int iReadCAM_CAL(u16 a_u2Addr, u32 ui4_length, u8 * a_puBuff)
{
    ...
    //CAM_CALDB("[CAM_CAL_5e2] iReadCAM_CAL!! \n");   
    //CAM_CALDB("[CAM_CAL_5e2] i2c_master_send \n");
    i4RetValue = i2c_master_send(g_pstI2Cclient, puReadCmd, 2);
	
    if (i4RetValue != 2)
    {
        CAM_CALDB("[CAM_CAL_5e2] I2C send read address failed!! \n");
        return -1;
    }

    //CAM_CALDB("[CAM_CAL_5e2] i2c_master_recv \n");
    i4RetValue = i2c_master_recv(g_pstI2Cclient, (char *)a_puBuff, ui4_length);
	CAM_CALDB("[CAM_CAL_5e2][iReadCAM_CAL] Read 0x%x=0x%x \n", a_u2Addr, a_puBuff[0]);
    if (i4RetValue != ui4_length)
    {
        CAM_CALDB("[CAM_CAL_5e2] I2C read data failed!! \n");
        return -1;
    } 

    //CAM_CALDB("[CAM_CAL_5e2] iReadCAM_CAL done!! \n");
    return 0;
}
...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   do 
   {
       CAM_CALDB("[CAM_CAL_5e2][iWriteData] Write 0x%x=0x%x \n",u4CurrentOffset, pBuff[0]);
       i4RetValue = iWriteCAM_CAL((u16)u4CurrentOffset, 1, pBuff[0]);
       if (i4RetValue != 0)
       {
            CAM_CALDB("[CAM_CAL_5e2] I2C iWriteData failed!! \n");
            return -1;
       }           
       u4IncOffset ++;
       i4ResidueDataLength --;
   ...
   CAM_CALDB("[CAM_CAL_5e2] iWriteData done\n" );
...
/*   if (ui4_offset + ui4_length >= 0x2000)
   {
      CAM_CALDB("[CAM_CAL_5e2] Read Error!! S-24CS64A not supprt address >= 0x2000!! \n" );
      return -1;
   }
   */
   ...
   do 
   {
	   ...
	   CAM_CALDB("[CAM_CAL_5e2][iReadData] Read 0x%x=0x%x,pBuff addr %p\n", u4CurrentOffset, *pBuff,pBuff);
	   //>2015/02/28-joubert.she
	   //>2015/02/09-joubert.she
	   if (i4RetValue != 0)
       {
            CAM_CALDB("[CAM_CAL_5e2] I2C iReadData failed!! \n");
            return -1;
       }           
       i4ResidueDataLength --;
...
static int Enb_OTP_Read(int enb){
	...
	if(enb){
		...
		{
			*pBuff = 0x04;
			iWriteData(0x0A00, 1, pBuff );
			*pBuff = page_count;
			iWriteData(0x0A02, 1, pBuff );
			*pBuff = 0x01;
			iWriteData(0x0A00, 1, pBuff );
                            mdelay(2);
                            otp_Start_Addr = 0x0A24;
			iReadCAM_CAL(otp_Start_Addr, 1, &flag);
			if(flag==0x1)
                            {         
                                    CAM_CALDB("[CAM_CAL] s5k5e2 otp enable success");
				return 1;
                              }
                            else
			{
                                    otp_Start_Addr = 0x0A04;
				iReadCAM_CAL(otp_Start_Addr, 1, &flag);
				if(flag==0x01){
                                        CAM_CALDB("[CAM_CAL] s5k5e2 otp enable success");
					return 1;
...
#endif
{
    ...
    else
    {
        ...
        if(NULL == pBuff)
        {
            CAM_CALDB("[CAM_CAL_5e2] ioctl allocate mem failed\n");
            return -ENOMEM;
        }
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                CAM_CALDB("[CAM_CAL_5e2] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    if(NULL == pWorkingBuff)
    {
     ...
     CAM_CALDB("[CAM_CAL_5e2] init Working buffer address 0x%8x  command is 0x%8x\n", (u32)pWorkingBuff, (u32)a_u4Command);
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:    
            CAM_CALDB("[CAM_CAL_5e2] Write CMD \n");
#ifdef CAM_CALGETDLT_DEBUG
        ...
        case CAM_CALIOC_G_READ:
            CAM_CALDB("[CAM_CAL_5e2] Read CMD \n");
#ifdef CAM_CALGETDLT_DEBUG            
            do_gettimeofday(&ktv1);
#endif 
            CAM_CALDB("[CAM_CAL_5e2] offset %d \n", ptempbuf->u4Offset);
            CAM_CALDB("[CAM_CAL_5e2] length %d \n", ptempbuf->u4Length);
			//<2015/02/09-joubert.she-[V10][Feature][Common][BSP][][] disable iReadData log 
			//<2015/02/28-joubert.she-[V10][Feature][Common][BSP][][]otp driver updated
	
            CAM_CALDB("[CAM_CAL_5e2] Before read Working buffer address 0x%p \n", pWorkingBuff);
	 		//<2015/02/28-joubert.she
			//>2015/02/09-joubert.she
            ...
            CAM_CALDB("[CAM_CAL_5e2] After read Working buffer data  0x%x \n", *pWorkingBuff);
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        CAM_CALDB("[CAM_CAL_5e2] to user length %d \n", ptempbuf->u4Length);
        CAM_CALDB("[CAM_CAL_5e2] to user  Working buffer value 0x%x \n", *(u32*)pWorkingBuff);
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pWorkingBuff);
            CAM_CALDB("[CAM_CAL_5e2] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
{
    CAM_CALDB("[CAM_CAL_5e2] CAM_CAL_Open\n");
    spin_lock(&g_CAM_CALLock);
    if(g_u4Opened)
    {
        spin_unlock(&g_CAM_CALLock);
		CAM_CALDB("[CAM_CAL_5e2] Opened, return -EBUSY\n");
        return -EBUSY;
    }
    else
    {
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        CAM_CALDB("[CAM_CAL_5e2] Allocate device no failed\n");
    ...
    {
        CAM_CALDB("[CAM_CAL_5e2] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        CAM_CALDB("[CAM_CAL_5e2] Allocate mem for kobject failed\n");
    ...
    {
        CAM_CALDB("[CAM_CAL_5e2] Attatch file operation failed\n");
...
static unsigned short force[] = {CAM_CAL_I2C_GROUP_ID, S5K5E2OTP_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};   
#else
//static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, S5K5E2OTP_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};   
#endif
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {             
int i4RetValue = 0;
    CAM_CALDB("[CAM_CAL_5e2] Attach I2C \n");
//    spin_lock_init(&g_CAM_CALLock);
	...
	//<2015/02/28-joubert.she-[V10][Feature][Common][BSP][][]otp driver updated
    g_pstI2Cclient->addr = S5K5E2FROTP_DEVICE_ID>>1;
	//>2015/02/28-joubert.she
    spin_unlock(&g_CAM_CALLock); // for SMP    
    
    CAM_CALDB("[CAM_CAL_5e2] g_pstI2Cclient->addr = 0x%8x \n",g_pstI2Cclient->addr);
    //Register char driver
    i4RetValue = RegisterCAM_CALCharDrv();

    if(i4RetValue){
        CAM_CALDB("[CAM_CAL_5e2] register char device failed!\n");
        return i4RetValue;
    }


    CAM_CALDB("[CAM_CAL_5e2] Attached!! \n");
    return 0;                                                                                       
} 
...
{
    CAM_CALDB("[CAM_CAL_5e2] CAM_CAL_probe!! \n");

    i2c_add_driver(&CAM_CAL_i2c_driver);
    return 0;
}
