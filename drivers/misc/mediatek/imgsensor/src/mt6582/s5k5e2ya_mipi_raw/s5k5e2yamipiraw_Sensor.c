...
void S5K5E2YA_MIPI_read_structure_version(struct S5K5E2YA_MIPI_otp_struct *otp)
{
	...
	{
		SENSORDB("[S5K5E2YA] PageCount=%d\n", PageCount);	
		S5K5E2YA_bytewrite_cmos_sensor(0x0a00, 0x04); 
		S5K5E2YA_bytewrite_cmos_sensor(0x0a02, PageCount); //otp enable read
		S5K5E2YA_bytewrite_cmos_sensor(0x0a00, 0x01); //otp enable read

		GroupFlag = S5K5E2YA_read_cmos_sensor(0x0a24);
		if(GroupFlag == 0x01)
		{
			if(PageCount==3)
				SENSORDB("[S5K5E2YA] otp Group=3\n");
			else
				SENSORDB("[S5K5E2YA] otp Group=1\n");
			otp->module_integrator_id = S5K5E2YA_read_cmos_sensor(0x0a26);
			otp->structure_version= S5K5E2YA_read_cmos_sensor(0x0a27);

			S5K5E2YA_bytewrite_cmos_sensor(0x0a00, 0x04);  //otp disable read
			S5K5E2YA_bytewrite_cmos_sensor(0x0a00, 0x00);  //otp disable read
			break;
		}
		else{
			GroupFlag = S5K5E2YA_read_cmos_sensor(0x0a04);
			if(GroupFlag == 0x01)
			{
				if(PageCount==3)
					SENSORDB("[S5K5E2YA] otp Group=2\n");
				else
					SENSORDB("[S5K5E2YA] otp Group=0\n");
				otp->module_integrator_id = S5K5E2YA_read_cmos_sensor(0x0a06);
				otp->structure_version= S5K5E2YA_read_cmos_sensor(0x0a07);
	...
	if(PageCount == 1)
	{
		SENSORDB("[S5K5E2YA] otp all value is zero");
		S5K5E2YA_bytewrite_cmos_sensor(0x0a00, 0x04);  //otp disable read
		S5K5E2YA_bytewrite_cmos_sensor(0x0a00, 0x00);  //otp disable read
	}

	SENSORDB("[S5K5E2YA] otp->ModuleID=0x%x\n", otp->module_integrator_id);	
	SENSORDB("[S5K5E2YA] otp->structure_version=0x%x\n", otp->structure_version);	
...
void S5K5E2YA_MIPI_read_otp_wb(struct S5K5E2YA_MIPI_otp_struct *otp)
{
	...
	for(PageCount = 3; PageCount>=2; PageCount--)
	{
		...
		if(GroupFlag == 0x01)
		{
			if(PageCount==3)
				...
				SENSORDB("[S5K5E2YA] otp Group=1\n");
			otp->module_integrator_id = S5K5E2YA_read_cmos_sensor(0x0a26);
			otp->R_to_G = S5K5E2YA_read_cmos_sensor(0x0a2e);
			otp->B_to_G = S5K5E2YA_read_cmos_sensor(0x0a2f);
			otp->Gr_to_Gb = S5K5E2YA_read_cmos_sensor(0x0a30);
			otp->unit_awb_r = S5K5E2YA_read_cmos_sensor(0x0a31);
			otp->unit_awb_gr = S5K5E2YA_read_cmos_sensor(0x0a32);
			otp->unit_awb_gb = S5K5E2YA_read_cmos_sensor(0x0a33);
			otp->unit_awb_b = S5K5E2YA_read_cmos_sensor(0x0a34);
			otp->golden_awb_r = S5K5E2YA_read_cmos_sensor(0x0a35);
			otp->golden_awb_gr = S5K5E2YA_read_cmos_sensor(0x0a36);
			otp->golden_awb_gb = S5K5E2YA_read_cmos_sensor(0x0a37);
			otp->golden_awb_b = S5K5E2YA_read_cmos_sensor(0x0a38);
			...
			if(GroupFlag == 0x01)
			{
				if(PageCount==3)
					...
					SENSORDB("[S5K5E2YA] otp Group=0\n");
				otp->module_integrator_id = S5K5E2YA_read_cmos_sensor(0x0a06);
				otp->R_to_G = S5K5E2YA_read_cmos_sensor(0x0a0e);
				otp->B_to_G = S5K5E2YA_read_cmos_sensor(0x0a0f);
				otp->Gr_to_Gb = S5K5E2YA_read_cmos_sensor(0x0a10);
				otp->unit_awb_r = S5K5E2YA_read_cmos_sensor(0x0a11);
				otp->unit_awb_gr = S5K5E2YA_read_cmos_sensor(0x0a12);
				otp->unit_awb_gb = S5K5E2YA_read_cmos_sensor(0x0a13);
				otp->unit_awb_b = S5K5E2YA_read_cmos_sensor(0x0a14);
				otp->golden_awb_r = S5K5E2YA_read_cmos_sensor(0x0a15);
				otp->golden_awb_gr = S5K5E2YA_read_cmos_sensor(0x0a16);
				otp->golden_awb_gb = S5K5E2YA_read_cmos_sensor(0x0a17);
				otp->golden_awb_b = S5K5E2YA_read_cmos_sensor(0x0a18);
	...
	SENSORDB("[S5K5E2YA] otp->ModuleID=0x%x\n", otp->module_integrator_id);	
	SENSORDB("[S5K5E2YA] otp->R_to_G=0x%x\n", otp->R_to_G);	
	SENSORDB("[S5K5E2YA] otp->B_to_G=0x%x\n", otp->B_to_G);	
	SENSORDB("[S5K5E2YA] otp->G_to_G=0x%x\n", otp->Gr_to_Gb);	

	SENSORDB("[S5K5E2YA] otp->golden_awb_r=0x%x\n", otp->golden_awb_r);	
	SENSORDB("[S5K5E2YA] otp->golden_awb_b=0x%x\n", otp->golden_awb_b);	
	SENSORDB("[S5K5E2YA] otp->golden_awb_gr=0x%x\n", otp->golden_awb_gr);	
}
...
void S5K5E2YA_MIPI_algorithm_otp_wb1(struct S5K5E2YA_MIPI_otp_struct *otp)
{
	...
	SENSORDB("[S5K5E2YA] R_to_G=%d\n", R_to_G);	
	SENSORDB("[S5K5E2YA] B_to_G=%d\n", B_to_G);	
	...
	SENSORDB("[S5K5E2YA] bg_ratio_typical=%d\n", bg_ratio_typical);	
	SENSORDB("[S5K5E2YA] rg_ratio_typical=%d\n", rg_ratio_typical);	
	...
	SENSORDB("[S5K5E2YA] R_gain=0x%x\n", otp->R_Gain);	
	SENSORDB("[S5K5E2YA] B_gain=0x%x\n", otp->B_Gain);	
	SENSORDB("[S5K5E2YA] G_gain=0x%x\n", otp->G_Gain);
}
...
void S5K5E2YA_MIPI_write_otp_wb(struct S5K5E2YA_MIPI_otp_struct *otp)
{
	...
	SENSORDB("[S5K5E2YA] R_GainH=0x%x\n", R_GainH);	
	SENSORDB("[S5K5E2YA] R_GainL=0x%x\n", R_GainL);	
	SENSORDB("[S5K5E2YA] B_GainH=0x%x\n", B_GainH);
	SENSORDB("[S5K5E2YA] B_GainL=0x%x\n", B_GainL);	
	SENSORDB("[S5K5E2YA] G_GainH=0x%x\n", G_GainH);	
	SENSORDB("[S5K5E2YA] G_GainL=0x%x\n", G_GainL);
...
void S5K5E2YA_write_shutter(kal_uint32 shutter)
{
    ...
    // C statr
    if(s5k5e2ya.sensorMode == SENSOR_MODE_ZSD_PREVIEW)
    {
...
void S5K5E2YAInitSetting(void)
{
	SENSORDB("s5k5e2yaInit Setting Start\n");                                                            
   ...
   S5K5E2YA_bytewrite_cmos_sensor(0x0B00,0x01); // GAS bypass
   S5K5E2YA_bytewrite_cmos_sensor(0x3400,0x00); // GAS bypass
...
*/
   S5K5E2YA_bytewrite_cmos_sensor(0x3407,0x04);
   S5K5E2YA_bytewrite_cmos_sensor(0x3408,0x00);
   S5K5E2YA_bytewrite_cmos_sensor(0x3409,0x0C);
   S5K5E2YA_bytewrite_cmos_sensor(0x340A,0xA4);
   S5K5E2YA_bytewrite_cmos_sensor(0x340B,0x04);
   S5K5E2YA_bytewrite_cmos_sensor(0x340C,0x00);
   S5K5E2YA_bytewrite_cmos_sensor(0x340D,0x10);
   S5K5E2YA_bytewrite_cmos_sensor(0x340E,0xC8);
   S5K5E2YA_bytewrite_cmos_sensor(0x3401,0x51);
   S5K5E2YA_bytewrite_cmos_sensor(0x3402,0x3D);
   S5K5E2YA_bytewrite_cmos_sensor(0x3403,0x03);
   S5K5E2YA_bytewrite_cmos_sensor(0x3404,0x29);
   S5K5E2YA_bytewrite_cmos_sensor(0x3405,0x04);
   S5K5E2YA_bytewrite_cmos_sensor(0x3406,0x32);
   S5K5E2YA_bytewrite_cmos_sensor(0x3458,0x03);
   S5K5E2YA_bytewrite_cmos_sensor(0x3459,0x82);
   S5K5E2YA_bytewrite_cmos_sensor(0x345A,0x04);
   S5K5E2YA_bytewrite_cmos_sensor(0x345B,0xD5);
...
*/
   ...
   S5K5E2YA_bytewrite_cmos_sensor(0x3409,0x0C);
   S5K5E2YA_bytewrite_cmos_sensor(0x340A,0xA4);
   S5K5E2YA_bytewrite_cmos_sensor(0x340B,0x02);
   S5K5E2YA_bytewrite_cmos_sensor(0x340C,0x02);
   S5K5E2YA_bytewrite_cmos_sensor(0x340D,0x08);
   S5K5E2YA_bytewrite_cmos_sensor(0x340E,0x64);
   S5K5E2YA_bytewrite_cmos_sensor(0x3401,0x51);
   S5K5E2YA_bytewrite_cmos_sensor(0x3402,0x3D);
   S5K5E2YA_bytewrite_cmos_sensor(0x3403,0x03);
   S5K5E2YA_bytewrite_cmos_sensor(0x3404,0x29);
...
UINT32 S5K5E2YAGetSensorID(UINT32 *sensorID)
{
	...
	S5K5E2YA_MIPI_read_structure_version(&current_otp);
