...
//<2015/01/26-joubert.she-[V10][Feature][Common][BSP][][] add torch light function for ats
#include <linux/i2c.h>
struct i2c_client   * flashlight_i2c_client = NULL;
#define TORCH_EN_GPIO GPIO13
#define FLASH_EN_GPIO GPIO12

  #define   GPIO_CAMERA_FLASH_MODE          TORCH_EN_GPIO
  #define   GPIO_CAMERA_FLASH_MODE_M_GPIO   GPIO_MODE_00
  #define   GPIO_CAMERA_FLASH_EN            FLASH_EN_GPIO
  #define   GPIO_CAMERA_FLASH_EN_M_GPIO     GPIO_MODE_00
  #define   I2C_USAGE_NUM                   1
//>2015/01/26-joubert.she
...
/****************************************************************************/
//<2015/02/09-joubert.she-[V10][Feature][Common][BSP][][] add torch light function for ats
//<2015/01/26-joubert.she-[V10][Feature][Common][BSP][][] add torch light function for ats
#if defined(V10_HW)
...
static ssize_t show_flashmode_sky81294(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	char databuf[2];

	PK_DBG("[flashlight] show_flashmode_sky81294\n");
	mdelay(2); 

	i2c_smbus_write_byte_data(flashlight_i2c_client,0x00, 0x0D); 
	i2c_smbus_write_byte_data(flashlight_i2c_client,0x01, 0x05); 
	i2c_smbus_write_byte_data(flashlight_i2c_client,0x03, 0x02); 
...
static ssize_t show_torchmode_sky81294(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	char databuf[2];
	...
	mdelay(2); 
		...
		i2c_smbus_write_byte_data(flashlight_i2c_client,0x02, 0x03);	
		i2c_smbus_write_byte_data(flashlight_i2c_client,0x03, 0x01);	

		mdelay(5000);

		i2c_smbus_write_byte_data(flashlight_i2c_client,0x03, 0x00);	


	return sprintf(buf, "%u\n", ret);
}

static ssize_t show_torchmode_on_sky81294(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	char databuf[2];
...
static ssize_t show_torchmode_off_sky81294(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	char databuf[2];
...
static DEVICE_ATTR(flashmode, 0664, show_flashmode_sky81294, NULL);
static DEVICE_ATTR(torchmode, 0664, show_torchmode_sky81294, NULL);
static DEVICE_ATTR(torchmode_on, 0664, show_torchmode_on_sky81294, NULL);
static DEVICE_ATTR(torchmode_off, 0664, show_torchmode_off_sky81294, NULL);
...
static int sky81294_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
...
//>2014/03/07-Yuting Shih.
	...
	printk("[flashlight_probe] sky81294_i2c_probe\n");	
...
static int  sky81294_i2c_remove(struct i2c_client *i2c)
{
	mt_set_gpio_mode(GPIO_CAMERA_FLASH_MODE, GPIO_CAMERA_FLASH_MODE_M_GPIO);
	mt_set_gpio_dir(GPIO_CAMERA_FLASH_MODE, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_CAMERA_FLASH_MODE, GPIO_OUT_ZERO);
...
static const struct i2c_device_id sky81294_i2c_ids[] = {
	{ "sky81294", 0 },
	{ },
};

static struct i2c_board_info __initdata i2c_sky81294={ I2C_BOARD_INFO("sky81294", (0x37))};////init in module
static struct i2c_driver sky81294_i2c_driver = {
	.driver = {
		   .name = "sky81294",
		   .owner = THIS_MODULE,
	},
	.id_table	= sky81294_i2c_ids,
	.probe		= sky81294_i2c_probe,
	.remove		= sky81294_i2c_remove,
};

#else
static int lm3648_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
//<2014/03/07-Yuting Shih. Add for flashlight control/turn on test.
//#if defined( LM3642_TEST_ENABLE )
//int i = 0;
...
//>2014/03/07-Yuting Shih.
	...
	printk("[flashlight_probe] lm3648_i2c_probe\n");	
...
static int lm3648_i2c_remove(struct i2c_client *i2c)
{
	mt_set_gpio_mode(GPIO_CAMERA_FLASH_MODE, GPIO_CAMERA_FLASH_MODE_M_GPIO);
	mt_set_gpio_dir(GPIO_CAMERA_FLASH_MODE, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_CAMERA_FLASH_MODE, GPIO_OUT_ZERO);
...
static const struct i2c_device_id lm3648_i2c_ids[] = {
	{ "lm3648", 0 },
	{ },
};
static struct i2c_board_info __initdata i2c_lm3648={ I2C_BOARD_INFO("lm3648", (0xc6>>1))};////init in module
...
//<2015/01/26-joubert.she-[V10][Feature][Common][BSP][][] add torch light function for ats
...
#if 1
	i2c_register_board_info( I2C_USAGE_NUM, &i2c_sky81294, 1 );
	...
	i2c_register_board_info( I2C_USAGE_NUM, &i2c_lm3648, 1 );
