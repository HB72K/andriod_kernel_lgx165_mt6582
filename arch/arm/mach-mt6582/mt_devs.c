...
//<2014/12/22-samhuang, porting e1-2 driver
#if defined(E12_HW)
static struct platform_device lm3533_leds_device = {
	.name = "leds-lm3533",
	.id = -1,
};
static struct platform_device sky81294_leds_device = {
	.name = "leds-sky81294",
	.id = -1,
};
#endif
...
//<2014/12/22-samhuang, portng e1-2 driver
#if defined(E12_HW)
	retval = platform_device_register(&lm3533_leds_device);
	if (retval != 0) {
		return retval;
    }
	//printk("lm3533 device LEDS register\n");
	
	retval = platform_device_register(&sky81294_leds_device);
	if (retval != 0) {
		return retval;
    }
#endif
